...
exports.setTranslator = exports.addTranslations = exports.setLanguage = exports.tr = void 0;
...
const findString = id => {
  ...
  return part.reduce((node, cur, index) => {
    ...
    if (isString) return node;else {
      if (node && node[cur]) {
        if (isLast && node[cur]._) return node[cur]._;else return node[cur];
      } else {
...
const regexReplace = (regex, str, callback) => {
  ...
  while ((match = re.exec(str)) !== null) {
    ...
    if (value) result = result.replace(match[0], value);
  }
...
let tr = (id, vars = {}) => {
  ...
  if (found) {
    // Find variables
    found = regexReplace(VAR_REGEX, found, match => vars[match]); // Find references
...
const addTranslations = lang => {
  translations = (0, _deepExtend.default)(translations, lang);
};
/**

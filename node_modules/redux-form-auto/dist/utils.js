...
exports.mergeInitialValues = exports.trPath = exports.objectTraverse = exports.arrLast = exports.arrayTrim = exports.objectIsEmpty = exports.forceArray = exports.schemaTypeEx = exports.schemaType = exports.getType = void 0;
...
const getType = thing => {
  ...
  if (kind == 'object' && thing instanceof _schema.Schema) return thing.getType();else return kind;
};
/**
...
const schemaTypeEx = type => {
  if (type instanceof _schema.Schema) return 'schema';else {
    const isArray = Array.isArray(type);
    if (isArray && type.length > 0 && type[0] instanceof _schema.Schema) return 'array';else return schemaType(type);
  }
};
/**
...
const forceArray = thing => Array.isArray(thing) ? thing : [thing];
/**
...
const arrayTrim = arr => {
  if (Array.isArray(arr)) {
    return arr.reduceRight((arr, msg) => {
      if (arr.length == 0 && objectIsEmpty(msg)) return arr;else return [msg].concat(_toConsumableArray(arr));
    }, []);
  } else {
    return arr;
  }
...
const objectTraverse = (obj, path, def) => {
  const [next, ...rest] = path;

  if (next in obj) {
    if (rest.length == 0) return obj[next];else return objectTraverse(obj[next], rest, def);
  } else return def;
};
/**
...
const trPath = (model, field, op) => {
  if (typeof op == 'undefined') return ['models', model, field].join('.');else return ['models', model, field, op].join('.');
};
/**
...
const mergeInitialValues = (initialValues = {}, schema) => {
  ...
  return fields.reduce((newDefaults, fieldName) => {
    ...
    if ('defaultValue' in fieldDef && !(fieldName in newDefaults)) newDefaults[fieldName] = fieldDef.defaultValue;

    if (fieldDef.type instanceof _schema.Schema) {
      const instanceCur = newDefaults[fieldName] || {};
      const instanceDefs = mergeInitialValues(instanceCur, fieldDef.type);
